...
router.get('/', async (req, res) => {
    try {
        const events = await database_1.default.eventType.findMany({
            orderBy: {
                name: 'asc'
            }
        });
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const event = await database_1.default.eventType.findUnique({
            where: { id },
            include: {
                bookings: {
                    include: {
...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const existingEvent = await database_1.default.eventType.findFirst({
            where: {
                name: {
                    equals: name.trim(),
                    mode: 'insensitive'
        ...
        const newEvent = await database_1.default.eventType.create({
            data: {
                name: name.trim()
            }
        });
...
router.put('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const existingEvent = await database_1.default.eventType.findUnique({
            where: { id }
        });
        if (!existingEvent) {
            return res.status(404).json({
        ...
        const updatedEvent = await database_1.default.eventType.update({
            where: { id },
            data: {
                name: name.trim()
            }
...
router.delete('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        await database_1.default.eventType.delete({
            where: { id }
        });
        res.json({
            success: true,
...
router.get('/popular', async (req, res) => {
    try {
        const popularEvents = await database_1.default.eventType.findMany({
            include: {
                _count: {
                    select: {
                        bookings: true
...
router.get('/search', async (req, res) => {
    try {
        ...
        const searchResults = await database_1.default.eventType.findMany({
            where: {
                name: {
                    contains: q.trim(),
                    mode: 'insensitive'
